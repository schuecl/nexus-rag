...
- Prompt-injection resistance for retrieved document content flowing into the generation model's context — untrusted content by construction, not yet defended against.
...
- Rename the `PUBLIC` access-scope value (FR-23) to something less easily confused with public-release content — e.g. `ALL_AUTHENTICATED` — cosmetic, no functional change, not urgent.
