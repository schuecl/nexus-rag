...
| Retrieval quality + leak check | `scripts/evaluate_retrieval.py` + `golden_queries.json` | `e2e.yml` (nightly, manual, or a PR labeled `needs-e2e`) | Full `docker compose up` → seed → golden-query run; fails on recall misses, on any pending/rejected/superseded leak, and (since #528) on an **access-scope** leak -- a document returned to a persona whose claims should exclude it, attributed by document id rather than filename (FR-26/FR-30/FR-32) |
