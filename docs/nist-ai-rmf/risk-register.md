...
| R-6 | Golden-query e2e (only leak/quality gate against a live stack) is label-opt-in; most PRs merge without it | M | M | Nightly run + carried trend store; mutation gate on filter modules; BDD scenarios on every PR | **Accepted pending formal record** | Issue #529 |
| R-7 | Prompt injection via curated corpus content into downstream generation. Live-probed history: direct override **resisted** (both probed models), exfiltration **resisted** (3B; 7B not recorded); DAN/roleplay **partially complied** (marker echo) → notice strengthened (#427); delimiter forgery **closed structurally** (#458); citation hijack mitigated by notice only (#457). Strengthened wording **not yet re-validated against live generation** | M | H | Delimiter neutralization (deterministic, unit-tested); SECURITY_NOTICE on both surfaces; curator-facing injection-marker advisory scan; access filter bounds blast radius to content the *querying user* is cleared for | **Mitigating — validation debt** | Issues #494, #532; `REQUIREMENTS.md` §11 P1 |
| R-8 | Platform-admin tier unmediated: `POSTGRES_USER` superuser and store-level credentials bypass every application control incl. audit append-only | M | H | Credential separation (NFR-3), per-service least-privilege roles verified live — but the superuser itself is outside them | **Open** | Issue #519 (role definition); G2 residual in `docs/roles-and-permissions.md` |
...
| R-14 | PyRIT red-team finding: multi-turn system-prompt extraction objective reached | M | M | Under triage as a security finding | **Open** | Issue #488 |
...
| Injection-probe live re-run with forgery/hijack fixtures; archive as red-team evidence | R-7 | Issues #494, #532 |
| Triage PyRIT extraction finding | R-14 | Issue #488 |
