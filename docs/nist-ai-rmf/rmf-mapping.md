...
| 2.1–2.3 Roles, responsibilities, executive accountability | Application roles implemented and covered by BDD/mocks, with retrieval-side enforcement and machine-identity least privilege validated live (`docs/roles-and-permissions.md`); governance-role table in `docs/governance.md`; **platform-admin tier undefined, no named accountable owner** — issue #519 | Partial; TBD (organizational) |
...
| 1.3 Assessment by independent parties | No assessor independent of the developer exists; the planned internal audit (governance-policy §9 — re-running this mapping against current `main`) is the closest mechanism, and its independence is an owner decision | Gap / TBD (organizational) |
