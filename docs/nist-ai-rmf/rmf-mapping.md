...
| 1.3 Assessment by independent parties | No assessor independent of the developer exists; the planned internal audit (governance-policy §9 — re-running this mapping against current `main`) is the closest mechanism, and its independence is an owner decision — issue #542 | Gap / TBD (organizational) |
...
| 2.6 Performance vs defined targets | FR-26 leak check: hard-fail, zero-tolerance (validated live). Latency: no agreed target (NFR-4 open) | Partial |
| 2.7 Security & resilience (red-team) | Live prompt-injection probe with dated findings and fix history (`REQUIREMENTS.md` §11 P1: #97 mixed result → #427 notice fix → #457/#458 delimiter-forgery closed structurally / citation-hijack notice-only); adversarial-input NFR-7 tests. #457/#458 fixes validated against live generation and the PyRIT extraction finding closed as a false positive with live evidence (PR 541); remaining residual is the DAN/roleplay marker echo (#427) and archiving the runs as snapshot evidence (issue #532) | Validated live (probe); Partial (archived evidence) |
