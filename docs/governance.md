...
| Reconnaissance-shaped query detection now exists — the in-repo detector ([#426](https://github.com/schuecl/nexus-rag/issues/426), validated live), the `NexusRagQueryAnomalyDetected`/`NexusRagQueryAnomalyDetectionStale` alert pair, and the SIEM runbook ([#436](https://github.com/schuecl/nexus-rag/issues/436)). Residuals: the SIEM-side rules are documentation sketches (untested by CI, thresholds untuned per deployment), nothing in this repo schedules the detector, and `boundary_mapping` cannot observe filter-edge probing (out-of-scope queries return successful empty results) | [siem-detection-runbook.md](siem-detection-runbook.md); scheduling tracked in [#527](https://github.com/schuecl/nexus-rag/issues/527) |
